...
class HMM():
	...
	def build_emission_probabilities(self, chorales, beatchorales, tmat):
		...
		for j, (beatchorale, chorale) in enumerate(zip(beatchorales,chorales)):
			...
			bass = extract_utils.extract_bassline(chorale)
			rna = beatchorale
...
class HMMMelody(HMM):
	...
	def build_transition_matrix_degrees(self, chorales):
		...
		for chorale in chorales:
			...
			rna = extract_utils.line_to_relative_pitches(chorale.parts['Soprano'], chorale.analyze('key'))
	...
	def build_emission_probabilities(self, chorales, beatchorales, tmat):
		...
		for j, (beatchorale, chorale) in enumerate(zip(beatchorales,chorales)):
			...
			# get bass line and rna for chorale
			bass = extract_utils.line_to_relative_pitches(chorale.parts['Bass'], chorale.analyze('key'))
			chords = chord_utils.degrees_on_beats(chorale)
			rna = beatchorale
			...
			for i in np.arange(len(bass)):
				...
				try:
					emission = str(chords[i]) + ', ' + str(bass[i])
				except:
					print('Bad Chorale: {}'.format(i))
				
				if em_mat.empty:
					em_mat.loc[rna[i], emission] = 1
				else:
					# if chord not seen before add to transition matrix
					if rna[i] not in em_mat.index:
						em_mat.loc[rna[i], :] = 0
					if emission not in em_mat.columns:
						em_mat.loc[:, emission] = 0
						
				# count emission from a particular meoldy note
				em_mat.loc[rna[i], emission] += 1
