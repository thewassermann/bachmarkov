...
class HMM():
	...
	def __init__(self, chorales):
		...
		self.emission_matrix = self.build_emission_probabilities(chorales, self.beat_chorales, selftransition_matrix)
		self.model = self.dfs_to_hmm(self.starting_states, self.emission_matrix, self.transition_matrix)


	def build_transition_matrix_degrees(chorales):
		"""
	    ...
	    for j in xrange(len(tmat.index)):
    ...
    def build_emission_probabilities(chorales, beatchorales, tmat):
    """
    ...
    em_mat = pd.DataFrame(0, index=tmat.index, columns=xrange(-1, 12))
    ...
    for j in xrange(len(em_mat.index)):
        em_mat.iloc[j,:] = em_mat.iloc[j,:] / rowSums[j]
        
    return(em_mat)


    def index_dict_to_prob_dict(d):
    	"""
	...
	def dfs_to_hmm(starting_df, emission_df, transition_df):
	    """
	    ...
	    # to prob dicts
	    em_pdict = index_dict_to_prob_dict(em_dict)
	    trans_pdict = index_dict_to_prob_dict(trans_dict)
