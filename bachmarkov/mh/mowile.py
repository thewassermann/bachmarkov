...
def MOWILECV(chorales, kfolds, budget, n_restarts, beta, alpha, weight_upper_bound, cd, iter_, T_0, cooling, p_ls, model_type):
	"""
	...
	for i in trange(kfolds, desc='Folds elapsed'):
		...
		for j in np.arange(len(training_)):
			...
			# MH
			if model_type == 'MH':
				train_model = mh_boolean.MCMCBooleanSampler(
			...
			# Gibbs
				...
				train_model = gibbs_boolean.GibbsBooleanSampler(
					training_[j].parts['Bass'],
					{
			...
			train_mowlie = MOWILE(training_[j], budget, n_restarts, beta, alpha, weight_upper_bound, train_model)
		...
		for k in np.arange(len(testing_)):
			...
			if model_type == 'MH':
				...
				test_model = mh_boolean.MCMCBooleanSampler(
				...
				test_model = gibbs_boolean.GibbsBooleanSampler(
					testing_[k].parts['Bass'],
					{
						'Alto' : (pitch.Pitch('g3'), pitch.Pitch('c5')),
						'Tenor' : (pitch.Pitch('c3'), pitch.Pitch('e4'))
			...
			test_model.run(iter_, False, False)
			
			derived_ll = test_model.log_likelihood()
			
			# true bach
			if model_type == 'MH':
				test_model.soprano = extract_utils.to_crotchet_stream(testing_[k].parts['Soprano'])
			elif model_type == 'Gibbs':
				test_model.alto = extract_utils.to_crotchet_stream(testing_[k].parts['Alto'])
				test_model.tenor = extract_utils.to_crotchet_stream(testing_[k].parts['Tenor'])
			true_ll = test_model.log_likelihood()
